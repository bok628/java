		...
		case 3:	
			...
			for(int i = 0; i < count; i++) {
				if(search.equals(name[i])) {
					...
					System.out.println("0. 취소 - 이전화면이동");
					System.out.println("-----------------");
					System.out.println("원하는 번호를 입력하세요.");
					choice = scan.nextInt();
...
//						input = scan.nextInt();
//						score[i][choice-1] = input;
//						total[i] = score[i][choice-1] + score[i][choice] + score[i][choice+1];
//						avg[i] = total[i] / 3.0;
//						System.out.printf("\n%d점으로 변경되었습니다.\n",score[i][choice-1]);
//						
//						break;
//					case 2:
//						System.out.printf("현재 %s 점수 : %d\n",title[choice+1],score[i][choice]);
//						System.out.println("변경점수를 입력하세요.");
//						input = scan.nextInt();
//						score[i][choice] = input;
//						total[i] = score[i][choice-1] + score[i][choice] + score[i][choice+1];
//						avg[i] = total[i] / 3.0;
//						System.out.printf("\n%d점으로 변경되었습니다.\n",score[i][choice]);
//						
...
//						break;
					case 0:
						System.out.println("점수 변경을 취소했습니다.");
						System.out.println();
						break;
					}
			...
			if(temp == 0) {
				System.out.printf("%s 학생이 존재하지 않습니다.",search);
				System.out.println();
			}
			break;
		case 0:
